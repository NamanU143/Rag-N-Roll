...
class SnowflakeCortexChat:
    ...
    def summarize_article(self, article_text, user_query):
        """Summarizes financial news with structured insights."""
        prompt = f"""
    ...
    def _execute_cortex_query(self, prompt):
        """Executes a Snowflake Cortex query with the given prompt."""
        sql_query = f"""
        ...
        try:
            ...
            logging.error(f"Error executing Cortex query: {e}")
    ...
    def stream_response(self, user_input):
        """Streams response from Cortex with a controlled delay while maintaining conversation history."""
        self.conversation_history.append(HumanMessage(content=user_input))
